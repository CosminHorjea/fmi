...
--ex 7
SELECT last_name "Nume angajat", rpad(to_char(hire_date),20,'X')"Data angajarii"
FROM employees
...
FROM employees
WHERE (salary + salary*commission_pct) >10000; -- daca nu am comision , conditia iese null
...
SELECT last_name, job_id, salary,
            DECODE(job_id,
            'IT_PROG', salary*1.1,  -- sunt if elseuri practic
...
SELECT last_name, job_id, salary,
            CASE job_id WHEN 'IT_PROG' THEN salary* 1.1  -- e gresit in pdf lab 2 , nu se pune "," dupa valoare
...
select employee_id,department_name
from employees e join departments d using(department_id); -- asta e doar cand coloanele se numesc la fel
...
from employees e, departments d
where e.department_id (+) = d.department_id; -- in ca z ca un employee nu are department
...
SELECT ang.employee_id Ang#, ang.last_name Angajat, sef.employee_id Mgr#,
        sef.last_name Manager
FROM employees ang, employees sef
WHERE ang.manager_id = sef.employee_id (+); -- unde vad (+) inseamna ca ii dau voi coloanei aleaia sa fie NULL
...
-- ex 24
SELECT ang.last_name, ang.hire_date, gates.hire_date
