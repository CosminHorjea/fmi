...
begin
  ...
  LOOP 
    ...
    END LOOP;
    DBMS_OUTPUT.PUT_LINE("A"  => '________________' /*IN VARCHAR2*/);
  END LOOP;
end;
/
...
SET SERVEROUTPUT ON;
--8 CATE filme sunt programate in fiecare sala
SELECT * FROM ADRESSES;

DECLARE
  TYPE numarFilme IS RECORD(numeCinema cinemas.name%TYPE,numarCamera rooms.ROOMID%TYPE,nrFilme NUMBER);
  TYPE vecFilme IS TABLE OF numarFilme;
  v_filme vecFilme;
  lastCinema CINEMAS.name%TYPE;
BEGIN
  SELECT c.name,r.roomNumber,COUNT(s.ScheduleID) "Filme"
  BULK COLLECT INTO v_filme
  FROM CINEMAS c JOIN ROOMS r USING (CINEMAID) JOIN SCHEDULE s USING(RoomID)
  GROUP BY c.name,r.roomNumber
  ORDER BY c.name;

  lastCinema := 'dummy';

  FOR i in v_filme.FIRST..v_filme.LAST LOOP
    IF lastCinema <> v_filme(i).numeCinema THEN 
      DBMS_OUTPUT.PUT_LINE('------------------------');
      DBMS_OUTPUT.PUT_LINE('La cinemaul ' || v_filme(i).numeCinema);
      lastCinema := v_filme(i).numeCinema;
    END IF;
    DBMS_OUTPUT.PUT_LINE('In sala '|| v_filme(i).numarCamera || ' ruleaza ' || v_filme(i).nrFilme || ' filme' );
  END LOOP;
END;
/
...
END;
/
SELECT d.name,SUM(p.price)
FROM DIRECTORS d JOIN MOVIES m  USING(DirectorID) JOIN SCHEDULE s USING(MovieID)
JOIN TICKETS t USING (SCHEDULEID) JOIN PRICES p USING (PriceID)
GROUP BY d.name;
